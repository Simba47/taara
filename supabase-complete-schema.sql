-- ============================================================
-- TAARA — COMPLETE DATABASE SCHEMA
...
CREATE TABLE IF NOT EXISTS public.actors (
  ...
  languages            text[] DEFAULT '{}', skills text[] DEFAULT '{}', accents text[] DEFAULT '{}',
...
CREATE TABLE IF NOT EXISTS public.casting_opportunities (
  ...
  casting_director text, deadline date, notes text, created_at timestamptz DEFAULT now()
);
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'actors_updated_at') THEN
    CREATE TRIGGER actors_updated_at BEFORE UPDATE ON public.actors FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'shortlists_updated_at') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'agency_updated_at') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'admin_profiles_updated_at') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'manager_profiles_updated_at') THEN
    CREATE TRIGGER manager_profiles_updated_at BEFORE UPDATE ON public.manager_profiles FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'talent_managers_updated_at') THEN
    CREATE TRIGGER talent_managers_updated_at BEFORE UPDATE ON public.talent_managers FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'join_requests_updated_at') THEN
    CREATE TRIGGER join_requests_updated_at BEFORE UPDATE ON public.join_requests FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
  END IF;
END $$;
...
-- Drop all policies cleanly before recreating
...
CREATE POLICY "filmography_insert_auth"  ON public.filmography FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "filmography_insert_anon"  ON public.filmography FOR INSERT WITH CHECK (user_id IS NULL);
CREATE POLICY "filmography_update"       ON public.filmography FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "filmography_delete"       ON public.filmography FOR DELETE USING (auth.uid() = user_id);

-- Portfolio
CREATE POLICY "portfolio_select"       ON public.portfolio FOR SELECT USING (true);
CREATE POLICY "portfolio_insert_auth"  ON public.portfolio FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "portfolio_insert_anon"  ON public.portfolio FOR INSERT WITH CHECK (user_id IS NULL);
CREATE POLICY "portfolio_update"       ON public.portfolio FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "portfolio_delete"       ON public.portfolio FOR DELETE USING (auth.uid() = user_id);
...
-- Admin profiles
CREATE POLICY "admin_select"  ON public.admin_profiles FOR SELECT USING (true);
CREATE POLICY "admin_insert"  ON public.admin_profiles FOR INSERT WITH CHECK (true);
CREATE POLICY "admin_update"  ON public.admin_profiles FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "admin_delete"  ON public.admin_profiles FOR DELETE USING (auth.uid() = user_id);

-- Manager profiles
CREATE POLICY "mp_select" ON public.manager_profiles FOR SELECT USING (true);
CREATE POLICY "mp_insert" ON public.manager_profiles FOR INSERT WITH CHECK (auth.uid() = user_id OR true);
...
-- Talent managers
CREATE POLICY "tm_self_select"   ON public.talent_managers FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "tm_admin_select"  ON public.talent_managers FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.admin_profiles ap WHERE ap.id = admin_id AND ap.user_id = auth.uid()));
CREATE POLICY "tm_admin_insert"  ON public.talent_managers FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM public.admin_profiles ap WHERE ap.id = admin_id AND ap.user_id = auth.uid()));
CREATE POLICY "tm_admin_update"  ON public.talent_managers FOR UPDATE USING (
  EXISTS (SELECT 1 FROM public.admin_profiles ap WHERE ap.id = admin_id AND ap.user_id = auth.uid()));
CREATE POLICY "tm_admin_delete"  ON public.talent_managers FOR DELETE USING (
  auth.uid() = user_id OR
  EXISTS (SELECT 1 FROM public.admin_profiles ap WHERE ap.id = admin_id AND ap.user_id = auth.uid()));

-- Work logs
CREATE POLICY "wl_manager_insert" ON public.work_logs FOR INSERT WITH CHECK (auth.uid() = manager_user_id);
CREATE POLICY "wl_manager_select" ON public.work_logs FOR SELECT USING (auth.uid() = manager_user_id);
CREATE POLICY "wl_admin_select"   ON public.work_logs FOR SELECT USING (
...
-- Join requests
CREATE POLICY "jr_manager_insert" ON public.join_requests FOR INSERT WITH CHECK (auth.uid() = manager_user_id);
CREATE POLICY "jr_manager_select" ON public.join_requests FOR SELECT USING (auth.uid() = manager_user_id);
...
CREATE INDEX IF NOT EXISTS admin_user_id_idx         ON public.admin_profiles(user_id);
...
CREATE INDEX IF NOT EXISTS mp_email_idx              ON public.manager_profiles(email);
CREATE INDEX IF NOT EXISTS tm_user_id_idx            ON public.talent_managers(user_id);
CREATE INDEX IF NOT EXISTS tm_admin_id_idx           ON public.talent_managers(admin_id);
CREATE INDEX IF NOT EXISTS tm_email_idx              ON public.talent_managers(email);
CREATE INDEX IF NOT EXISTS wl_manager_idx            ON public.work_logs(manager_user_id);
CREATE INDEX IF NOT EXISTS wl_created_at_idx         ON public.work_logs(created_at DESC);
CREATE INDEX IF NOT EXISTS jr_manager_idx            ON public.join_requests(manager_user_id);
CREATE INDEX IF NOT EXISTS jr_admin_idx              ON public.join_requests(target_admin_id);

-- ── POSTGRES FUNCTION: create_manager_account ─────────────────
-- Creates manager auth user server-side (never touches admin session)

DROP FUNCTION IF EXISTS public.create_manager_account(text,text,text,text,uuid);
...
BEGIN
  ...
  VALUES (v_user_id, '00000000-0000-0000-0000-000000000000', lower(trim(p_email)),
...
-- ── POSTGRES FUNCTION: approve_join_request ───────────────────
...
CREATE POLICY "storage_public_read"   ON storage.objects FOR SELECT USING (bucket_id IN ('headshots','portfolio','avatars'));
CREATE POLICY "headshot_upload_auth"  ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'headshots');
CREATE POLICY "headshot_update_auth"  ON storage.objects FOR UPDATE USING (bucket_id = 'headshots' AND auth.uid()::text = (storage.foldername(name))[1]);
CREATE POLICY "headshot_delete_auth"  ON storage.objects FOR DELETE USING (bucket_id = 'headshots' AND auth.uid()::text = (storage.foldername(name))[1]);
CREATE POLICY "portfolio_upload"      ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'portfolio');
...
-- ============================================================
-- DONE. Go to Supabase → Authentication → Providers → Email
-- → Disable "Confirm email" so admins can log in immediately.
-- ============================================================
