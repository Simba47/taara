...
export function EditActorModal({ actor, open, onOpenChange }: EditActorModalProps) {
  ...
  const [reelUrl, setReelUrl] = useState(actor.reel_url || "");
      ...
      <DialogContent className="sm:max-w-2xl glass-card border-border max-h-[90vh] overflow-y-auto">
        ...
        <Tabs defaultValue="basic" className="mt-2">
          ...
          <TabsContent value="bio" className="space-y-4">
            ...
            <FieldGroup label="Showreel URL"><Input value={reelUrl} onChange={e => setReelUrl(e.target.value)} placeholder="https://vimeo.com/..." className="font-body text-[13px] rounded-xl h-10 bg-background border-border" /></FieldGroup>
          </TabsContent>
          ...
          <TabsContent value="notes" className="space-y-4">
            <FieldGroup label="Private manager notes (not visible to actors or clients)">
              <Textarea value={managerNotes} onChange={e => setManagerNotes(e.target.value)} rows={6} placeholder="Internal notes, reminders, contact info…" className="font-body text-[13px] rounded-xl bg-background border-border resize-none" />
            </FieldGroup>
          </TabsContent>
        </Tabs>
