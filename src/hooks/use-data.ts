...
// ── Work log helper (fire-and-forget, never throws) ───────────
async function logWork(userId: string, action: string, details?: string) {
  try {
    await supabase.from("work_logs").insert({ manager_user_id: userId, action, details });
...
export function useActorBySlug(slug: string) {
  return useQuery({
    ...
    enabled: !!slug,
    queryFn: async () => {
      // Public profile: explicitly exclude contact_phone and contact_email
      const { data, error } = await supabase
        .from("actors")
        .select(`
          id, name, age, gender, type, status, location,
          height, hair, eyes, bio, reel_url, reel_urls,
          headshot_url, slug, profile_visible, color,
          languages, skills, accents,
          filmography(*), portfolio(*), works(*)
        `)
        .eq("slug", slug)
        .maybeSingle();
      if (error) throw error;
...
export function useCreateActor() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      logWork(data.user_id, "Added actor", data.name);
    },
  });
}
...
export function useCreateEvent() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      logWork(data.user_id, "Added calendar event", `${data.type}: ${data.title}`);
    },
  });
}
...
export function useCreateShortlist() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      logWork(data.user_id, "Created shortlist", data.name);
    },
  });
}
...
// ─────────────────────────────────────────────────────────────
// ACTOR AVAILABILITY (persistent calendar dates)
// ─────────────────────────────────────────────────────────────

export type DateStatus = "available" | "booked" | "frozen" | "unavailable";

export function useActorAvailability(actorId: string) {
  const { user } = useAuth();
  return useQuery({
    queryKey: ["availability", actorId],
    enabled: !!user && !!actorId,
    staleTime: 1000 * 60 * 2,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("actor_availability")
        .select("date, status")
        .eq("actor_id", actorId)
        .eq("user_id", user!.id);
      if (error) throw error;
...
export function useSaveActorAvailability() {
  const { user } = useAuth();
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ actorId, dateStatuses }: { actorId: string; dateStatuses: Record<string, DateStatus> }) => {
      // Delete all existing, re-insert current state
      await supabase.from("actor_availability").delete().eq("actor_id", actorId).eq("user_id", user!.id);
      const rows = Object.entries(dateStatuses).map(([date, status]) => ({
        actor_id: actorId, user_id: user!.id, date, status,
      }));
      if (rows.length > 0) {
        const { error } = await supabase.from("actor_availability").insert(rows);
        if (error) throw error;
      }
    },
    onSuccess: (_, vars) => qc.invalidateQueries({ queryKey: ["availability", vars.actorId] }),
  });
}
