...
const ActorProfile = () => {
  ...
  if (isLoading) return (
    <div className="max-w-[1100px] mx-auto px-8 py-10 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6"><Skeleton className="lg:col-span-2 h-96 rounded-2xl" /><Skeleton className="h-96 rounded-2xl" /></div>
    </div>
  );
        ...
        <div className="lg:col-span-2 space-y-6">
          <Tabs defaultValue="overview" className="w-full">
            ...
            <TabsContent value="overview" className="space-y-6 mt-6">
              ...
              {/* Contact info — manager only, never shared publicly */}
              {(actor.contact_phone || actor.contact_email) && (
                <div className="glass-card p-6 border border-primary/20 bg-primary/5">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-[10px] font-body text-muted-foreground px-2 py-0.5 rounded-full bg-accent border border-border">Manager only · not shared</span>
                  </div>
                  <div className="space-y-2">
                    {actor.contact_phone && (
                      <div className="flex items-center gap-3">
                        <span className="text-[12px] text-muted-foreground font-body w-16 shrink-0">Phone</span>
                        <a href={`tel:${actor.contact_phone}`} className="text-[13px] font-body font-medium text-primary hover:underline">{actor.contact_phone}</a>
                      </div>
                    )}
                    {actor.contact_email && (
                      <div className="flex items-center gap-3">
                        <span className="text-[12px] text-muted-foreground font-body w-16 shrink-0">Email</span>
                        <a href={`mailto:${actor.contact_email}`} className="text-[13px] font-body font-medium text-primary hover:underline">{actor.contact_email}</a>
                      </div>
                    )}
                  </div>
                </div>
              )}
              {actor.manager_notes && actor.manager_notes !== "Registered via invite link." && (
            ...
            return (
              <div className="glass-card p-5">
                <h2 className="font-display text-lg font-bold mb-3">
                  Showreel{reels.length > 1 ? `s (${reels.length})` : ""}
                </h2>
                <div className="space-y-4">
                  {reels.map((url, i) => (
                    <div key={i}>
                      {reels.length > 1 && (
                        <p className="text-[11px] font-body text-muted-foreground font-semibold mb-2 uppercase tracking-wider">
                          Reel {i + 1}
                        </p>
                      )}
                      {url.includes("youtube.com") || url.includes("youtu.be") ? (
                        <div className="aspect-video rounded-xl overflow-hidden border border-border">
                          <iframe src={url.replace("watch?v=", "embed/").replace("youtu.be/", "youtube.com/embed/")}
                            ...
                            allowFullScreen title={`Showreel ${i + 1}`} />
                        </div>
                        ...
                        <a href={url} target="_blank" rel="noopener noreferrer"
                          className="flex items-center gap-2 text-[13px] font-body font-medium text-primary hover:text-primary/80 transition-colors border border-border rounded-xl px-4 py-2.5 hover:bg-accent/30">
                          <ExternalLink className="h-4 w-4" strokeWidth={1.5} />
                          {reels.length > 1 ? `Watch Showreel ${i + 1}` : "Watch Showreel"}
                        </a>
                      )}
                    </div>
                  ))}
