...
const Dashboard = () => {
  ...
  return (
    <div className="max-w-[1100px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 animate-fade-in">
      <div className="mb-8">
        ...
        <div className="mb-8">
          ...
          <div className="glass-card divide-y divide-border overflow-hidden border-amber-200/50">
            {pendingLoading ? (
              ...
              pending.map((actor) => {
                const headshot = actor.headshot_url;
                const portfolioPhotos: any[] = actor.portfolio || [];
                const allPhotos = [
                  ...(headshot ? [headshot] : []),
                  ...portfolioPhotos.map((p: any) => p.image_url || p.imageUrl).filter(Boolean),
                ];
                const initials = actor.name.split(" ").map((n: string) => n[0]).join("").slice(0, 2).toUpperCase();

                return (
                  <div key={actor.id} className="border border-border rounded-xl overflow-hidden mx-4 mb-2">
                    ...
                    <div className="flex items-center gap-3 px-4 py-3 hover:bg-accent/20 transition-colors">
                      ...
                      >
                        {headshot ? (
                          ...
                          <div className="w-11 h-11 rounded-xl flex items-center justify-center text-xs font-display font-bold ring-1 ring-border"
                      ...
                      <button
                        ...
                        onClick={() => setExpandedId(expandedId === actor.id ? null : actor.id)}
                      >
                        <p className="text-[13px] font-body font-semibold truncate">{actor.name}</p>
                        <div className="flex items-center gap-2 flex-wrap">
                          {actor.type && <span className="text-[10px] font-body text-muted-foreground px-1.5 py-0.5 rounded-full bg-accent">{actor.type}</span>}
                          {actor.age && <span className="text-[11px] text-muted-foreground font-body">{actor.age}y</span>}
                          {actor.gender && <span className="text-[11px] text-muted-foreground font-body">{actor.gender}</span>}
                          {actor.location && <span className="text-[11px] text-muted-foreground font-body">· {actor.location}</span>}
                          {allPhotos.length > 0 && <span className="text-[10px] text-muted-foreground font-body flex items-center gap-1"><Camera className="h-3 w-3" />{allPhotos.length}</span>}
                          <span className="text-[10px] text-primary font-body font-medium">{expandedId === actor.id ? "▲ hide" : "▼ view details"}</span>
                        </div>
                      </button>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <Button size="sm"
                          ...
                          onClick={() => handleApprove(actor.id, actor.name)} disabled={claimActor.isPending}>
                          <Check className="h-3.5 w-3.5 mr-1" /> Approve
                        </Button>
                        <button onClick={() => handleReject(actor.id, actor.name)}
                    ...
                    {expandedId === actor.id && (
                      <div className="border-t border-border bg-accent/10 animate-fade-in">
                        {/* Details grid */}
                        <div className="px-4 py-3 grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-1.5 text-[12px] font-body">
                          {actor.contact_phone && <div><span className="text-muted-foreground">Phone </span><span className="font-semibold">{actor.contact_phone}</span></div>}
                          {actor.contact_email && <div><span className="text-muted-foreground">Email </span><span className="font-semibold">{actor.contact_email}</span></div>}
                          {actor.height && <div><span className="text-muted-foreground">Height </span><span className="font-medium">{actor.height}</span></div>}
                          {actor.hair && <div><span className="text-muted-foreground">Hair </span><span className="font-medium">{actor.hair}</span></div>}
                          {actor.eyes && <div><span className="text-muted-foreground">Eyes </span><span className="font-medium">{actor.eyes}</span></div>}
                          {actor.languages?.length > 0 && <div className="col-span-2 sm:col-span-1"><span className="text-muted-foreground">Languages </span><span className="font-medium">{actor.languages.join(", ")}</span></div>}
                          {actor.skills?.length > 0 && <div className="col-span-2 sm:col-span-3"><span className="text-muted-foreground">Skills </span><span className="font-medium">{actor.skills.join(", ")}</span></div>}
                          {actor.bio && <div className="col-span-2 sm:col-span-3 text-muted-foreground italic mt-1">"{actor.bio.slice(0, 160)}{actor.bio.length > 160 ? "…" : ""}"</div>}
                        </div>

                        {/* Photos */}
                        {allPhotos.length > 0 && (
                          <div className="px-4 pb-3">
                            <p className="text-[10px] font-body text-muted-foreground font-semibold uppercase tracking-wider mb-2">{allPhotos.length} photo{allPhotos.length !== 1 ? "s" : ""}</p>
                        ...
                        {actor.filmography?.length > 0 && (
                          <div className="px-4 pb-4">
                            <p className="text-[10px] font-body text-muted-foreground font-semibold uppercase tracking-wider mb-2">Filmography</p>
                            <div className="flex flex-wrap gap-2">
                              {actor.filmography.map((f: any) => (
                                <span key={f.id} className="text-[11px] font-body px-2.5 py-1 rounded-lg bg-card border border-border">
                                  {f.title}{f.role ? ` — ${f.role}` : ""}{f.year ? ` (${f.year})` : ""}
                                </span>
                              ))}
                            </div>
                          </div>
